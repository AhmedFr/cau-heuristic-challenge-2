...
class Agent:  # Do not change the name of this class!
    ...
    def decide_new_village(self, board: GameBoard, time_limit: float = None) -> Tuple[int, int]:
        ...
        while time() < time_limit:
            ...
            if random() < exp((current_fitness - next_fitness) / TEMPERATURE) :
              escaped_maximas.append([current, current_fitness])
              current = next_pos
              current_fitness = next_fitness
        ...
        # return lowest fitness stored in escaped_maximas or current
        return min(escaped_maximas, key=lambda x: x[1])[0] if escaped_maximas else current
